...
def _convert_to_example(filename, tensor_buffer):
  example = tf.train.Example(features=tf.train.Features(feature={
    'tensor/encoded': _bytes_feature(tensor_buffer)
  }))
  return example
...
def _process_file(filename):
  ...
  if (tensor.shape != config.input_shape) and (tensor.shape != config.label_shape):
    print tensor.shape
  assert (tensor.shape == config.input_shape) or \
         (tensor.shape == config.label_shape)

  tensor_data = tensor.tostring()
  return tensor_data
...
def _process_files_batch(thread_index, ranges, name, filenames, num_shards):
  ...
  for s in range(num_shards_per_batch):
    ...
    for i in files_in_shard:
      ...
      tensor_buffer = _process_file(filename)

      example = _convert_to_example(filename, tensor_buffer)
