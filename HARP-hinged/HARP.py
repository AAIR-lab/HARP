'''
For set A experiments.

Experiments: For each start/goal pair, compute motion plans 10 times.
            showing that initially it takes more time but once we have the graph it takes a lot less time.
            done for 20 different start_goal pairs.
...
# np.random.seed(1632013593) #random seed
...
def load_pd():
    ...
    for i in range(5):
        for j in range(5):
            if pd is None:
                pd = np.squeeze(np.load(
                    os.path.join('network', 'results', str(test_sample) + "_" + str(j) + "_" + str(i) + '.npy')))
            else:
                t = np.squeeze(np.load(
                    os.path.join('network', 'results', str(test_sample) + "_" + str(j) + "_" + str(i) + '.npy')))
                pd = np.hstack((pd, t))
        if final_pd is None:
            final_pd = pd
    ...
    pd = np.squeeze(pd)
    xy_pd = pd[:,:,0]
    mask = np.ma.masked_where(xy_pd < 0.4, xy_pd)
    xy_pd[mask.mask] = 0.0
    xy_pd[~mask.mask] = 1.0
    pd[:,:,0] = xy_pd
...
class LL(object):
    ...
    def __init__(self,env,jointlimits,pd,n,m,s=[],g=[],pda = None, guided=False):
        ...
        # self.pda = pda
        # if guided: 
        #     init_x, init_y = self.convert_to_xy_bin(self.s)
        #     goal_x, goal_y = self.convert_to_xy_bin(self.g)
        #     new_pd, path = self.pda.generate_distribution([init_x,init_y],[goal_x,goal_y])
        #     self.pda.plot(path)
        #     self.pd[:,:,0] = new_pd
        self.guided = guided
    ...
    def sample_pd(self):
        ...
        # y_dof, x_dof = self.pda.generate_sample()

        dof_3 = np.random.choice(range(10),p = self.pd[y_dof,x_dof,1:11]/ np.sum( self.pd[y_dof,x_dof,1:11]))
        dof_4 = np.random.choice(range(10),p = self.pd[y_dof,x_dof,11:]/ np.sum( self.pd[y_dof,x_dof,11:]))
        ...
        dof_values = self.convert_sample((x_dof,y_dof,dof_3,dof_4))
    ...
    def compound_step_constrained(self,p1,p2):
        if self.dist(p1,p2,False) <= 0.05:
            ...
            dist = min(dist,0.1)
            new_x = p1[0] + dist * math.cos(theta + (phi/2.0))
            new_y = p1[1] + dist * math.sin(theta + (phi/2.0))
    ...
    def get_next_sample(self):
        ...
        while True:
            ...
            if not self.collides(sample):
                self.env.plot3(points=[sample[0], sample[1], 0.50], pointsize=0.03, colors=array((0,1,0)), drawstyle=1)
                break
            else:
                self.env.plot3(points=[sample[0], sample[1], 0.50], pointsize=0.03, colors=array((1, 0, 0)),
                               drawstyle=1)
        ...
        if self.currentstate == 'connected graphs':
            ...
            if len(path) > 0:
                ...
                if visualize:
                    ...
                    prev = None
                    for point in path:
                        self.trace_2.append(self.env.plot3(points=[point[0], point[1], 0.3], pointsize=0.05, colors=array((0,0,0)), drawstyle=1))
                        if prev is not None:
...
def setupRobot(robot):
    if robot.GetName() == 'robot1' or robot.GetName() == "bot":
        ...
        # robot.SetDOFLimits([-10, -10, -np.pi], [10, 10, np.pi])
        robot.SetDOFLimits([-12.5, -12.5, -np.pi], [12.5, 12.5, np.pi])
    elif robot.GetName() == 'lshaped':
        robot.SetDOFVelocities([0.5, 0.5, 0.5, 0.5, 0.5])
        robot.SetActiveDOFs([0,1,2,3])
        # robot.SetDOFLimits([-12.5, -12.5, -3*np.pi/4.0, -np.pi/2], [12.5, 12.5, 5*np.pi/4.0, np.pi/2])
        robot.SetDOFLimits([-12.5, -12.5, -np.pi, -np.pi/2], [12.5, 12.5, np.pi, np.pi/2])
...
def main():
    ...
    if test_sample in ['10.2.1', '10.2.2', '10.0.1', '10.0.2']:
        ...
        start = [1.6, 1.0, 0]
    elif test_sample in ['30.0.1']:
        start = [-9.1, 9, 0]
        # start = [9.1, 9, 0]
        goal = [6.3, -7.1, 0]

    elif test_sample in ["31.0.1"]:
        start = [7.5, 0, 0]
        goal = [-7.5, -7.1, 0]

    elif test_sample in ["32.0.1"]:
        start = [-8, -5.1, 0]
        goal = [-7,4, 0]

    elif test_sample in ["40.0.1"]:
        start = [8,8,0,0]
        goal = [5, -7, 0,0]

    elif test_sample in ["41.0.1"]:
        start = [-4.01,11.01,0,0]
        # start = [10,10,0]
        # goal = [6, -10, 0]
        goal = [3.78, -10.03, 0,0]

    elif test_sample in ["42.0.1"]:
        start = [10, 5, 0,0]
        goal = [8, -10, 0,0]
        p = 0.48

    elif test_sample in ["43.0.1"]:
        start = [5, 10, 0,0]
        goal = [6, -10, 0,0]
    ...
    # goal =  [6.3, -7.1, 0]  #30.0.1
    # goal = [-8.5,-7.1, 0] #31.0.1
    # goal = [8.8, 9, 0] # 32.0.1
    with env:
    ...
    results_file = open(
        os.path.join('results', 'env' + envnum, 'setC' , 'setC.csv'), 'a')
    # robot.SetActiveDOFValues(goal)
    robot.SetActiveDOFValues(start)
    print("start: ", start)
    print("goal: ", goal)
    ...
    # print "Time to build voronoi: {}".format(time.time() - start_time)
    pda = PDAugmenter(pd[:,:,0],p,inp,0.4)
    pda.plot()
    if pd_mode == "learn":
        m = 0
        n = 1000
    ...
    if mode == 'llp':
        ...
        for i in xrange(NUMRUNS):
            current  = i
            problem = LL(env,jointlimits,pd,n,m,start,goal,pda=pda,guided=guided)
            starttime = time.time()
