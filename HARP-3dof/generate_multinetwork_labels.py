...
class LabelGenerator():
    ...
    def buildLabel(self):  
        ...
        # dof_3_count = dof_3_count / float(len(self.fullpath))
        ...
        pd = np.concatenate([pd_xy,dof_3_channel],axis = -1)
        pd = np.nan_to_num(pd)
    ...
    def mergeLabel(self, channels_tensor, gaze_tensor):
        ...
        assert lbl.shape[2] == 11 # should have 3 channels (0: critical region gaze, 1-2: dof values)
    ...
    def custom_sm(self,pd,threshold):
        ...
        mask = np.ma.masked_where( ((pd < mean - (6 * std)) | (pd > mean + (6* std))), pd )
        pd_copy = pd.copy()
        pd_copy[mask.mask] = 0
        m = np.max(pd_copy)
        new_pd = pd / m
    ...
    def generate(self):
        ...
        # gaze_tensor = self.buildSM(lbl[:,:,0],env_tensor,self.threshold)
