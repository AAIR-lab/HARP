...
class MotionPlansGenerator:
    ...
    def getStartGoal(self, numpaths):
        ...
        if self.envnum == 5.1:
            ...
            if self.runnum%2 == 0:
                start = [1.25, s_yvalue, sd1_value]
                goal = [-1.25, g_yvalue, gd1_value]
            else:
                start = [-1.25, s_yvalue, sd1_value]
                goal = [1.25, g_yvalue, gd1_value]
            ...
            if self.runnum%2 == 0:
                start = [1.3,s_yvalue, sd1_value]
                goal = [-1.3,g_yvalue, gd1_value]
            else:
                start = [-1.3,s_yvalue, sd1_value]
                goal = [1.3,g_yvalue, gd1_value]

        elif self.envnum == 8.0:
            start = [1.5,0.59,sd1_value]
            goal = [-1.5,0.59,gd1_value]
            ...
            g_yvalue = np.random.uniform(-0.5,0.5)
            if self.runnum%2 == 0:
                start = [1.4, s_yvalue, sd1_value]
                goal = [-1.4, g_yvalue, gd1_value]
            else:
                start = [-1.4, s_yvalue, sd1_value]
                goal = [1.4, g_yvalue, gd1_value]
            ...
            if self.runnum%2 == 0:
                start = [s_xvalue, s_yvalue, sd1_value]
                goal = [g_xvalue, g_yvalue, gd1_value]
            else:
                start = [g_xvalue, g_yvalue, gd1_value]
                goal = [s_xvalue, s_yvalue, sd1_value]

        elif self.envnum == 18.2 or self.envnum == 22.0:
            s_yvalue = np.random.uniform(-0.5,0.5)
            g_yvalue = np.random.uniform(-0.5,0.5)
            if self.runnum%2 == 0:
                start = [1.6, s_yvalue, sd1_value]
                goal = [-1.6, g_yvalue, gd1_value]
            else:
                start = [-1.6, s_yvalue, sd1_value]
                goal = [1.6, g_yvalue, gd1_value]
            ...
            while True:
                ...
                start = [startx, starty, sd1_value]
                goal = [goalx, goaly, gd1_value]
    ...
    def generate(self):
        ...
        #             if not (self.env.CheckCollision(self.robot) or self.env.CheckCollision(self.robot)):
        #                 break
...
if __name__ == "__main__":
    ...
    env = Environment()
    env.Load("./envs3d/{}.xml".format(envnum))

    robot = env.GetRobots()[0]
    bounds = [[-2.5, -2.5, -2.5], [2.5, 2.5, 2.5]]

    generator = MotionPlansGenerator(robot,envnum,bounds,None,runnum)
    generator.generate()
