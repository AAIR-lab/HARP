...
class InputGenerator():
    ...
    def pixelInObstacle(self, pindex):
        ...
        body.InitFromBoxes(np.array([[aveX, aveY, 1.5, self.pixelwidth, self.pixelwidth, 1.0]]), True) # make sure z-coordinate is bounded by shortest wall height
    ...
    def buildInput(self, inppath, goal_loc):
        ...
        assert inp.shape[2] == 6 # should have 7 channels (0-2: env image, 3-7: goal dof values)
    ...
    def setupRobot(self):
        if self.robot.GetName() == 'robot1' or self.robot.GetName() == "bot":
            ...
            self.robot.SetDOFLimits(self.bounds[0], self.bounds[1])
    ...
    def generate(self, goal_loc,i,j):
        '''
        ...
        inppath = os.path.join('test', "env"+str(self.envnum), str(self.envnum)+"."+str(self.runnum) + "_" + str(i) + "_" + str(j) + '.npy')
...
if __name__ == '__main__':
    ...
    envPath = 'envs3d/43.0.dae'
    envnum = 43.0
    runnum = 1
    # goal_loc = [2.,   1.,   1.57] # 8.0.1
    # goal_loc = [-2., -1.,  0.,  0.] # 8.0.2
    # goal_loc = [-2.,  1. , 0.,  0.] # 8.0.3
    # goal_loc = [ 2., -1.,  0.,  0.] # 8.0.4
    # goal_loc = [-0.96970837, -0.82476306, -2.42265842] # 10.2.1
    # goal_loc = [2.,   1.6,  1.57, 0.  ] # 21.0.1
    # goal_loc = [6.3, -7.1, 0] # 30.0.1
    # goal_loc = [-8.5,-7.1, 0] # 31.0.1
    # goal_loc = [8.8, 9, 0] # 32.0.1
    # goal_loc = [9, -7, 0] # 40.0.1
    goal_loc = [6, -10, 0] # 41.0.1
    ...
    # envmax_rot = 3.14
    #
    # bounds = np.array([[-10.1, -10.1, -3.14], [10.1, 10.1, 3.14]]) # all random envs are built using the same base world, thus they have the same world bounds
    # envmin = np.array([-10.1, -10, -2.5])
    ...
    bounds = np.array([[-12.6, -12.6, -3.14], [12.6, 12.6,
                                               3.14]])  # all random envs are built using the same base world, thus they have the same world bounds
    envmin = np.array([-12.6, -12.6, -2.5])
    envmax = np.array([12.6, 12.6, 2.5])

    n_steps = 5

    step_x = ( envmax_x - envmin_x ) / float(n_steps)
    step_y = ( envmax_y - envmin_y ) / float(n_steps)

    for i in range(n_steps):
        for j in range(n_steps):
            cmix = envmin_x + (step_x * i)
            cmiy = envmin_y + (step_y * j)
            cmax = cmix + step_x
            cmay = cmiy + step_y
            bounds = np.array([[cmix,cmiy,-3.14],[cmax,cmay,3.14]])
            envmin = np.array([cmix,cmiy,-3.14])
            envmax = np.array([cmax,cmay,3.14])


            # if cmix <= goal_loc[0] <= cmax and cmiy <= goal_loc[1] <= cmay:
            #     temp_goal = goal_loc
            # else:
            temp_goal = [(cmix+cmax)/2.0, (cmiy+cmay)/2.0, 0]
            generator = InputGenerator(robot,envnum,bounds,[envmin,envmax],runnum)
            generator.generate(temp_goal,i,j)
