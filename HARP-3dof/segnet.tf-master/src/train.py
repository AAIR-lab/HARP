...
tf.app.flags.DEFINE_string('ckpts_path', './ckpts/model.ckpt', 'Train checkpoint file path')
...
tf.app.flags.DEFINE_float('learning_rate', 1e-03, 'learning rate for optimizer')
...
def train():
    ...
    X, y = read_tfrecords(FLAGS.batch, train_file, train_labels_file)
    X_val, y_val = read_tfrecords(FLAGS.batch, val_file, val_labels_file, 7, 0)

    # Write channels to summary
    ...
    if y_val:
        y_gaze_val = tf.slice(y_val, [0,0,0,0], [FLAGS.batch, 224, 224, 1])
        y_dof1_val = tf.slice(y_val, [0,0,0,1], [FLAGS.batch, 224, 224, 1])
        y_dof2_val = tf.slice(y_val, [0,0,0,2], [FLAGS.batch, 224, 224, 1])
        tf.summary.image('y_gaze_val', y_gaze_val, max_outputs=FLAGS.batch)
        tf.summary.image('y_dof1_val', y_dof1_val, max_outputs=FLAGS.batch)
        tf.summary.image('y_dof2_val', y_dof2_val, max_outputs=FLAGS.batch)

    model = utils.get_model(config.model, 'train')
    predictions, logits = model.inference(X, 'train', max_outputs=FLAGS.batch)
    loss = ae_loss(predictions, logits, y, 'train')
...
def main(argv=None):
    start = time.time()
    train()
    print("Training completed in ", str(time.time() - start))
