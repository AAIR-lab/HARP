...
class UNetV2(Autoencoder):
    ...
    def inference(self, X, runtype, max_outputs):
        ...
        '''
        with tf.variable_scope('dof_layer',reuse=tf.compat.v1.AUTO_REUSE):
            dof_layer_1 = self.deconv(deconv1_2,[self.output_channels-1,self.output_channels],'dof_layer_1')
        logits_dof1 = dof_layer_1 
        ...
        predictions = tf.concat([predictions_cr, predictions_dof1], axis=3)
        activations = tf.concat([tf.reshape(activations_cr,[FLAGS.batch,224,224,1]),tf.reshape(activations_dof1,[FLAGS.batch,224,224,10])],axis = 3)

        # Write activation images to summary
        tf.summary.image(runtype + '_activation_cr', activations_cr, max_outputs=max_outputs)

        logits = tf.concat([logits_cr,logits_dof1],axis =3)
    ...
    def loss(self,logits, predictions, labels, runtype):
        ...
        total_loss = tf.reduce_mean(3 * loss_cr + loss_dof1)
